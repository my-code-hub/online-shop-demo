...
import static com.sgt.online.shop.cookies.Cookies.USER_SHOPPING_CART_COOKIE_NAME;
...
public class ProductsController {
    ...
    public String addProductToCart(@PathVariable Long productId,
                                   ...
                                   @CookieValue(name = USER_SHOPPING_CART_COOKIE_NAME, required = false) String userCartId,
                                   AddProductToCartForm form,
                                   HttpServletResponse response,
                                   Model model) {
        System.out.println("add to cart product: " + productId + " and quantity " + form);
        System.out.println("Cart cookie ID: " + userCartId);

        model.addAttribute("productForm", new AddProductToCartForm());
        model.addAttribute("isSuccess", true);

        //Example of how we can generate unique ID and set cookie for user
        addShoppingCartCookieToResponse(response);
        return "shop/product-details";
    }

    private void addShoppingCartCookieToResponse(HttpServletResponse response) {
        String userCartIdentifier = UUID.randomUUID().toString(); // example how we can create unique identifier
        Cookie cookie = CookieUtils.createCookie(USER_SHOPPING_CART_COOKIE_NAME, userCartIdentifier);
